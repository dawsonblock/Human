...
FastAPI  (/api/*)                                               │
      ...
      └── GET /runs/{id}/export ──► SQLiteBackend.export_run_bundle()
```
...
- **Atomic commits**: `SQLiteBackend.apply_cycle_transition()` writes state, sequential events, and mirrored artifact indices in a single `BEGIN IMMEDIATE` transaction.
- **RunStore Protocol**: The runtime is decoupled from SQLite via a type-safe `RunStore` protocol, allowing for `:memory:` or file-backed implementations.
...
- **Path Hardening**: Pre-resolution traversal rejection (`..` segment checks) and multi-root validation via `HUMAN_ALLOWED_ROOTS`.
...
| SQLite persistence | ✅ | State, events, approvals, artifacts (indexed) |
| Atomic transactions | ✅ | Cycle commit rolls back all tables on any error |
| Path Hardening | ✅ | Pre-resolution traversal rejection, `:memory:` support |
| Export Bundles | ✅ | Self-contained JSON snapshots per run |
...
│       │       └── echo.py
│       ├── storage/
│       │   ├── interfaces.py       # RunStore protocol
│       │   ├── sqlite_backend.py   # Atomic SQLite implementation
│       │   ├── migrations.py       # Schema versioning + FK constraints
│       │   ├── paths.py            # Hardened path resolution + root enforcement
│       │   └── export.py           # JSON bundle generation
│       ├── state/
│       │   ├── models.py           # AgentStateV2_1, Goal, Plan, PlanStep, etc.
│       │   ├── sqlite_store.py     # Legacy store (kept for compatibility)
...
| Storage Hardening (atomicity, artifact merging, path traversal rejection, :memory:) | `test_storage_*.py` |
| SQLite Pragmas (Foreign Keys, busy timeout, WAL mode) | `test_storage_pragmas.py` |
...
| Variable | Effect |
|---|---|
| `HUMAN_DATA_DIR` | Base directory for runtime data and default workspace. |
| `HUMAN_DB_PATH` | Explicit path to SQLite database. Supports `:memory:`. |
| `HUMAN_ALLOWED_ROOTS` | Multi-root path string (e.g. `root1:root2`) for tool access. |
